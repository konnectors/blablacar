...
function start(fields) {
  ...
  return authenticate(fields.email, fields.password)
        .then(() => {request = requestFactory({cheerio: true})})
        .then(getHistory)
        .then(list => getDatas(list))
        .then(bills => saveBills(bills, fields, {identifiers: "blablacar",
...
async function getDatas(list) {
  ...
  for (let billInfo of list) {
    ...
    const trip = $('h2[class=u-left]','section[class="main-block"]').text().trim()
          .replace('→','-').replace(/\s/g,'')
    const start = $('div[class=row-content]').eq(1).text().trim().replace(/\s\s+/g, ' ')
    const places = $('div[class=row-content]').eq(2).text().trim()
    const amount = $('div[class=row-content]').eq(3).text().replace(/\s/g,'')
    const driver = $('a','div[class=driver-info]').text().trim().replace(/\s\s+/g, ' ')

    stream = generatePdf(trip, billInfo['date'], start, places, amount,
                         driver, billInfo['link'])
    bills.push({
      filestream: stream._doc,
      filename: (date.format('YYYY-MM-DD') + '_' + trip + '_' + amount + '.pdf').replace(' ',''),
      vendor: 'BlaBlacar',
...
function generatePdf(trip, date, start, places, amount, driver, link) {
  ...
  doc.cell({paddingBottom: 0.5 * pdf.cm}).text().add('Informations de voyage Blablacar.fr',
                        {font: helveticaBoldFont, fontSize:14})
  ...
  doc.cell({paddingBottom: 0.5 * pdf.cm}).text().add('Généré automatiquement par le connecteur cozy blablacar.')
  doc.end()
  return doc
}
...
async function getHistory() {
  ...
  let again = true
  while (again) {
    try { $ = await request(paymentsUrl + '/' + i) }
    catch(error) {
